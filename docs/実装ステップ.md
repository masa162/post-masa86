# 実装ステップ - post-masa86

**目標**: 確実に動作するブログを段階的に構築する

## Phase 1: 最小構成（Hello World）- 最優先

### ✅ チェックリスト

- [ ] **Step 1.1**: Next.js 15プロジェクト初期化
  - package.json作成
  - 必要最小限の依存関係のみ
  - wrangler.tomlは作成しない（重要）

- [ ] **Step 1.2**: 基本ファイル作成
  - tsconfig.json
  - next.config.js（シンプル版）
  - tailwind.config.ts
  - .gitignore

- [ ] **Step 1.3**: 最小限のページ作成
  - app/layout.tsx
  - app/globals.css
  - app/page.tsx（"Hello World"のみ、D1アクセスなし）

- [ ] **Step 1.4**: ローカルビルド確認
  - `npm install`
  - `npm run build`
  - エラーなくビルド完了

- [ ] **Step 1.5**: ローカル動作確認
  - `npm run dev`
  - http://localhost:3000 で "Hello World" 表示

- [ ] **Step 1.6**: GitHubにプッシュ
  - git commit
  - git push origin main

- [ ] **Step 1.7**: Cloudflare Pages初回デプロイ
  - Dashboard → Pages → Connect to Git
  - Build command: `npx @cloudflare/next-on-pages`
  - Build output: `.vercel/output/static`
  - デプロイ成功確認

- [ ] **Step 1.8**: 本番URL確認
  - https://post-masa86.pages.dev/
  - "Hello World"が表示される
  - 500エラーが出ない

---

## Phase 2: D1接続テスト

### ✅ チェックリスト

- [ ] **Step 2.1**: D1データベース作成
  - `npx wrangler d1 create post-masa86-db`
  - database_idをメモ

- [ ] **Step 2.2**: schema.sql作成
  - postsテーブル定義
  - インデックス定義

- [ ] **Step 2.3**: D1テーブル作成
  - ローカル: `npx wrangler d1 execute --local --file=./schema.sql`
  - 本番: `npx wrangler d1 execute --remote --file=./schema.sql`

- [ ] **Step 2.4**: テスト用データ1件追加
  - test-data.sql作成（1件のみ）
  - D1にインポート

- [ ] **Step 2.5**: lib/db.ts作成
  - getD1()関数（process.env.DB使用）
  - createMockD1()関数
  - getPosts()メソッドのみ実装

- [ ] **Step 2.6**: types/cloudflare.ts作成
  - グローバル型定義

- [ ] **Step 2.7**: app/page.tsx更新
  - dynamic = 'force-dynamic' 追加
  - revalidate = 0 追加
  - runtime = 'edge' 追加
  - db.getPosts()を呼び出し
  - 1件の記事タイトルを表示

- [ ] **Step 2.8**: ローカルビルド確認
  - `npm run build`
  - エラーなし

- [ ] **Step 2.9**: GitHubにプッシュ

- [ ] **Step 2.10**: Cloudflare Dashboard設定
  - Bindings → D1 database追加（DB = post-masa86-db）
  - Runtime → Compatibility flags追加（nodejs_compat）

- [ ] **Step 2.11**: 再デプロイ実行
  - Retry deployment

- [ ] **Step 2.12**: 本番確認
  - https://post-masa86.pages.dev/
  - 1件の記事タイトルが表示される
  - 500エラーが出ない

---

## Phase 3: 基本的な記事表示

### ✅ チェックリスト

- [ ] **Step 3.1**: components作成
  - Header.tsx（シンプル）
  - PostCard.tsx（1件表示用）

- [ ] **Step 3.2**: lib/markdown.ts作成
  - parseMarkdown()
  - formatDate()

- [ ] **Step 3.3**: app/page.tsx拡張
  - Header表示
  - PostCard表示

- [ ] **Step 3.4**: 記事詳細ページ作成
  - app/[slug]/page.tsx
  - dynamic/revalidate/runtime設定
  - db.getPostBySlug()実装

- [ ] **Step 3.5**: ローカル確認
  - トップページ表示
  - 記事詳細ページ表示

- [ ] **Step 3.6**: GitHubにプッシュ

- [ ] **Step 3.7**: 本番確認
  - トップページOK
  - 記事詳細ページOK

---

## Phase 4: Hugoデータ移行

### ✅ チェックリスト

- [ ] **Step 4.1**: scripts/migrate-hugo.ts作成

- [ ] **Step 4.2**: マイグレーション実行
  - `npm run migrate:hugo`
  - migrate-data.sql生成（49件）

- [ ] **Step 4.3**: D1にインポート
  - ローカル: `npx wrangler d1 execute --local --file=./migrate-data.sql`
  - 本番: `npx wrangler d1 execute --remote --file=./migrate-data.sql`

- [ ] **Step 4.4**: データ確認
  - `SELECT COUNT(*) FROM posts`
  - 49件確認

- [ ] **Step 4.5**: ローカル確認
  - トップページで5件表示
  - 記事詳細で全記事アクセス可能

- [ ] **Step 4.6**: 本番確認
  - 49件全て表示される

---

## Phase 5: 管理画面（最小機能）

### ✅ チェックリスト

- [ ] **Step 5.1**: lib/auth.ts作成
  - checkBasicAuth()実装

- [ ] **Step 5.2**: app/api/posts/route.ts作成
  - GET実装（一覧取得）
  - POST実装（新規作成）

- [ ] **Step 5.3**: app/admin/layout.tsx作成
  - Basic認証チェック
  - シンプルなナビゲーション

- [ ] **Step 5.4**: app/admin/page.tsx作成
  - 記事一覧表示のみ

- [ ] **Step 5.5**: ローカル確認
  - /admin にアクセス
  - Basic認証（mn/39）でログイン
  - 記事一覧が表示される

- [ ] **Step 5.6**: GitHubにプッシュ

- [ ] **Step 5.7**: 本番確認
  - /admin が動作する

- [ ] **Step 5.8**: app/admin/new/page.tsx作成
  - 新規作成フォーム

- [ ] **Step 5.9**: 記事作成テスト
  - 管理画面で記事作成
  - slug 0050として保存される

- [ ] **Step 5.10**: 編集・削除機能追加
  - app/admin/edit/[id]/page.tsx
  - app/api/posts/[id]/route.ts

---

## Phase 6: デザイン完成

### ✅ チェックリスト

- [ ] **Step 6.1**: Sidebar.tsx作成

- [ ] **Step 6.2**: SearchBox.tsx作成

- [ ] **Step 6.3**: globals.css完成
  - masa86.comのCSS完全再現

- [ ] **Step 6.4**: レスポンシブ対応確認
  - デスクトップ
  - タブレット
  - スマホ

---

## Phase 7: 付加機能

### ✅ チェックリスト

- [ ] **Step 7.1**: タグページ実装
  - app/tags/[tag]/page.tsx

- [ ] **Step 7.2**: 検索ページ実装
  - app/search/page.tsx

- [ ] **Step 7.3**: サイトマップ実装
  - app/sitemap.xml/route.ts

- [ ] **Step 7.4**: robots.txt実装
  - app/robots.txt/route.ts

---

## 各Phaseの成功基準

### Phase 1 成功基準
✅ https://post-masa86.pages.dev/ で "Hello World" が表示される

### Phase 2 成功基準
✅ https://post-masa86.pages.dev/ で記事タイトル1件が表示される

### Phase 3 成功基準
✅ 記事詳細ページが開ける

### Phase 4 成功基準
✅ 49件全ての記事が表示される

### Phase 5 成功基準
✅ 管理画面で記事作成ができる

### Phase 6 成功基準
✅ デザインがmasa86.comと同じになる

### Phase 7 成功基準
✅ 全機能が動作する

---

## 重要な注意事項

### 絶対に守ること

1. **wrangler.tomlは作成しない**
2. **各Stepで必ず動作確認**
3. **エラーが出たら次に進まない**
4. **Cloudflare Dashboard設定を優先**
5. **clasicjlit実装を参考にする**

### 各Step完了の判定

- ローカルビルド成功
- ローカル動作確認
- 本番デプロイ成功
- 本番動作確認

---

**まずはPhase 1から確実に進めます。**

